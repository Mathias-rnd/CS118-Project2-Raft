...
func (n *Node) doLeader() stateFunction {
    ...
    // REMOVED: NOOP Entry (Bug A fix) - strictly speaking not required and can break log matching tests.
    ...
    for {
        ...
        case clientRequestMsg := <-n.clientRequest:
            // Handle both Registration and State Machine commands identically:
            // 1. Create Log Entry
            // 2. Store Log
            // 3. Register Reply Channel
            // 4. Replicate (Heartbeat)
            // 5. Reply ONLY after commit (in processLogEntry)

            req := clientRequestMsg.request
            var entryType CommandType
            var cacheID string

            if req.StateMachineCmd == 0 && len(req.Data) == 0 {
                entryType = CommandType_CLIENT_REGISTRATION
                // Use a unique cache ID for registration too, so we can look up the reply channel later
            ...
            // Register the reply channel BEFORE storing/replicating
            n.requestsMutex.Lock()
            n.requestsByCacheID[cacheID] = append(n.requestsByCacheID[cacheID], clientRequestMsg.reply)
            n.requestsMutex.Unlock()
            ...
            // Optimistically update our own matchIndex
            n.LeaderMutex.Lock()
            n.matchIndex[n.Self.Id] = entry.Index
            n.LeaderMutex.Unlock()
    ...
    for _, peer := range nodes {
        ...
        go func(p *RemoteNode) {
            ...
            if reply.Success {
                ...
                // Commit Logic
                // Iterate from LastLogIndex down to CommitIndex + 1
                for N := n.LastLogIndex(); N > n.CommitIndex.Load(); N-- {
                    // Rule: Leader can only commit entries from its current term
                    if n.GetLog(N).TermId != currTerm {
                        continue
                    }
                    ...
                    if count > len(nodes)/2 {
                        ...
                        // Apply entries
                        // Note: We do this inside the lock to ensure order, 
                        // but be careful if processLogEntry blocks.
                        start := n.LastApplied.Load() + 1
                        for i := start; i <= N; i++ {
                            n.processLogEntry(i)
                            n.LastApplied.Store(i)
                        }
                        break // Found the highest committed index, stop checking lower ones
                    }
                }
                n.LeaderMutex.Unlock()
